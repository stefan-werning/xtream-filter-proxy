...
def _insert_pending_probe(db, kind, item_id, priority=20):
    with db.cursor() as cur:
        cur.execute(
            "INSERT INTO probe_state (kind, item_id, status, attempts, priority) VALUES (?, ?, 'pending', 0, ?)",
            (kind, item_id, priority),
        )
